...
RSpec.describe MinHeap, type: Class do
  ...
  describe "#delete(data)" do
    ...
    it "properly deletes root with no children" do
      tree.delete(root, root.title)
      expect(tree.find(tree.root, root.title)).to be_nil
    end

    it "properly deletes root with one child" do
      tree.insert(root, martian)
      tree.delete(root, root.title)
      expect(tree.find(tree.root, root.title)).to be_nil
      expect(tree.find(tree.root, martian.title)).to_not be_nil
    end

    it "properly deletes root with two children" do
      tree.insert(root, empire)
      tree.insert(root, mad_max_2)
      tree.insert(root, hope)
      tree.insert(root, martian)
    ...
    it "properly deletes a left node with one child" do
      tree.insert(root, braveheart)
      tree.insert(root, pacific_rim)
      tree.delete(root, braveheart.title)
      expect(tree.find(root, pacific_rim.title)).to_not be_nil
